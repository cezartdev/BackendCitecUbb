...
export const PdfTransformWorkOrder= (numero_folio, fecha_solicitud, fecha_entrega,observacion,cliente,direccion, provincia, comuna, descripcion) => {
    const relativePdfPath = path.posix.join("pdf", "workOrders", `workOrder_${numero_folio}.pdf`);
    const absolutePdfPath = path.join(__dirname, "..", "../", relativePdfPath);
    ensureDirectoryExistence(absolutePdfPath);
    const fecha = new Date().toISOString().split("T")[0];
    const doc = new PDFDocument({ margin: 50 });
    ...
    // Sección del número de folio (en esquina superior derecha, negrita)
    doc.fontSize(16).font('Helvetica-Bold').text(`Folio N°${numero_folio}`, { align: "right" });
    doc.moveDown(1);

    // Información de la orden de trabajo (con un diseño más limpio)
    doc.fontSize(12).font('Helvetica').text(`Fecha: ${fecha}`, { align: "left" });
    doc.text(`Cliente: ${cliente}`, { align: "left" });
    doc.text(`Direccion: ${direccion}`, { align: "left" });
    doc.text(`Provincia: ${provincia}`, { align: "left" });
    doc.text(`Comuna: ${comuna}`, { align: "left" });
    doc.text(`Descripcion: ${descripcion}`, { align: "left" });
    doc.text(`Fecha Solicitud: ${fecha_solicitud}`, { align: "left" });
    doc.text(`Fecha Entrega: ${fecha_entrega}`, { align: "left" });
    doc.text(`Observacion: ${observacion}`, { align: "left" });
    doc.moveDown(1);
