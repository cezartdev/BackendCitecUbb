...
// Crear orden de trabajo
    ...
    static async create(
        ...
        try {
            ...
            const [idComuna] = await db.execute<RowDataPacket[]>(queryComuna, [comuna]);
            ...
            for (const servicio of servicios) {
                ...
                const [servicioDB] = await db.execute<RowDataPacket[]>(queryServicios, [
                    servicio.nombre,
                ]);
                if (!servicioDB[0]) {
                    const errors = [
    ...
    static async update(
        ...
        try {
            ...
            const [estados] = await db.execute<RowDataPacket[]>(queryEstados, [estado]);
            if (!estados[0]) {
                const errors = [
                    {
                        type: "field",
